...
| **LLM** | `Google Gemini 1.5 Flash` | 고속 답글 생성 및 데이터 분석 |
...
├── .env                  # 환경 변수 (Google API Key)
├── .gitignore            # Git 제외 설정
├── data/                 # 데이터 저장소
│   ├── templates.json    # 답변 템플릿 및 말투 학습 데이터
│   ├── menu_info.json    # 가게 메뉴 정보
│   └── saved_reviews.json # (Git 제외) 완료된 리뷰 기록
├── chroma_db/            # (Git 제외) 로컬 벡터 DB 저장소
└── src/
    ├── data_manager.py   # 데이터 로드/저장/초기화 로직
    ├── models.py         # KoBERT 및 Gemini 모델 로더
    ├── rag.py            # ChromaDB 검색 및 임베딩 로직
    ├── utils.py          # 공통 유틸리티
    ├── workflow.py       # LangGraph 파이프라인 (핵심 로직)
    └── ui/               # [UI 모듈 분리]
        ├── styles.py     # CSS 스타일 정의
        ├── sidebar.py    # 사이드바 (설정 및 리셋)
        ├── cards.py      # 리뷰 관리 및 생성 탭
        ├── dashboard.py  # 통계 대시보드 탭
        ├── menu.py       # 메뉴 정보 관리 탭
        └── training.py   # 말투 학습 탭
```